...
public class MatchScoreDto(Player firstPlayer, Player secondPlayer) {
    public Player FirstPlayer { get; set; } = firstPlayer;
    public Player SecondPlayer { get; set; } = secondPlayer;

    private readonly Dictionary<int, PlayerScoresDto> _playersScores = new() {
        { firstPlayer.Id, new PlayerScoresDto() },
        { secondPlayer.Id, new PlayerScoresDto() }
    };
    ...
    public bool IsMatchFinished {
        get {
            ...
            for (int i = 0; i < NumberOfSets; i++) {
                var setPair = _playersScores.Values.Select(ps => ps.Sets[i]).ToList();
    ...
    public bool TryGetScoreComponent<T>(
        string componentName, int playerId, out T? component, object?[]? parameters = null
    ) {
        component = default;
    ...
    public void SetScoreComponents<T>(string componentName, T firstPlayerComponent, T secondPlayerComponent) {
